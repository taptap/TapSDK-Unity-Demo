...
namespace TapTap.Common.Editor
{
    public static class TapCommonCompile
    {
        ...
        public static string GetUnityPackagePath(string unityPackageName) {
                ...
                if (pkg.name == unityPackageName) {
                    if (pkg.source == PackageSource.Local)
                        ...
                        var remotePackagePath =
                            TapFileHelper.FilterFileByPrefix(Application.dataPath + "/Library/PackageCache/", $"{unityPackageName}@");
            ...
            string moduleName, string[] bundleNames, string target, string projPath, PBXProject proj)
        {
            ...
            var remotePackagePath = GetUnityPackagePath(modulePackageName);
