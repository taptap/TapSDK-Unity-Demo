...
#define TapMomentSDK_VERSION_NUMBER @"32701001"
#define TapMomentSDK_VERSION        @"3.27.1"
...
+ (void)setDefaultConfig:(TapMomentConfig *)config;
...
+ (void)directlyOpen:(NSString *)page extras:(NSDictionary *)extras;

/// 发布动态
/// @param content 发布的动态内容
/// @warning `moment`参数：动态内容为视频请选择`TapMomentVideoData`, 动态内容为图片时请选择 `TapMomentImageData`。
+ (void)publish:(TapMomentPostData *_Nonnull)content;
