...
public class LoginScene : MonoBehaviour
{
    ...
    private void OnGUI()
    {
        ...
        if (GUI.Button(new Rect((Screen.width - btnGap) / 2 - btnWidth, btnTop, btnWidth, btnHeight), "退出登录", style))
		{
            TDSUser.Logout();
        }

        if (GUI.Button(new Rect((Screen.width - btnGap) / 2 + btnGap, btnTop, btnWidth, btnHeight), "获取用户信息", style))
		{
            getUserInfo();
		}
